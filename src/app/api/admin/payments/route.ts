  ...
  try {
    ...
    if (search) {
      ...
      filteredData = (data || []).filter((payment: any) => {
        const normalizedName = normalizeVietnamese(payment.Attendees?.full_name || '').toLowerCase();
        const normalizedEmail = normalizeVietnamese(payment.Attendees?.email || '').toLowerCase();
